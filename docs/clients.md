...
See an overview of the configured clients, including revocation status:
    ...
    docker run --rm -it -v $OVPN_DATA:/etc/openvpn koehn/openvpn ovpn_listclients
...
Execute the following to generate the configuration for all clients:
    ...
    docker run --rm -it -v $OVPN_DATA:/etc/openvpn --volume /tmp/openvpn_clients:/etc/openvpn/clients koehn/openvpn ovpn_getclient_all
...
Revoke `client1`'s certificate and generate the certificate revocation list (CRL) using [`ovpn_revokeclient`](/bin/ovpn_revokeclient) script :
    ...
    docker run --rm -it -v $OVPN_DATA:/etc/openvpn koehn/openvpn ovpn_revokeclient client1
...
You can optionally pass `remove` as second parameter to ovpn_revokeclient to remove the corresponding crt, key and req files :
    ...
    docker run --rm -it -v $OVPN_DATA:/etc/openvpn koehn/openvpn ovpn_revokeclient client1 remove
